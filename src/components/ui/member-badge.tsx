...
export function MemberBadge({ name, ensName, badge, link, className }: MemberBadgeProps) {
  ...
  return (
    <button onClick={handleClick} className={cn("text-left cursor-pointer min-w-[220px]", className)} aria-label={`Open ${ensName} profile`}>
      <CornerCard variant="member" interactive contentClassName="px-[18px]" cornerClassName="-top-0.5 -right-0.5" className="bg-card rounded-xl rounded-tr-2xl hover:bg-card/60 transition-all duration-200">
        <div className="group relative flex items-center justify-end gap-3">
          <div className="flex-1 min-w-0 text-right">
            <h4 className="font-semibold text-sm truncate group-hover:text-primary transition-all duration-200 text-right">
              {ensName}
            </h4>
          </div>
